...
p("This app provides modelled layers for different aspects of benthic biodiversity. Models are based on ",tags$a(href="https://en.wikipedia.org/wiki/Random_forest", "random forest") ," modelling of point sample data from the ",tags$a(href="https://sway.office.com/HM5VkWvBoZ86atYP?ref=Link/", "OneBenthic")," big data initiative.  Understanding of benthic biodiversity is vital for effective decision concerning marine spatial planning, marine licensing, conservation, ",tags$a(href="https://www.marine-ecosystems.org.uk/Research_outcomes/Natural_Capital", "marine natural capital,"),
"  and ",tags$a(href="https://www.researchgate.net/publication/334250681_Marine_Net_Gain_Moving_towards_a_practical_framework_and_metric_for_the_marine_environment", "net gain."))
...
p("Using the drop-down boxes on the left of the screen, select a category of modelled layer (currently Structure or Species distibution model (SDM)). Next select an individual layer to display.","The map window will then update. A variety of associated model outputs are shown in tabs (",tags$b("Layer Info, Characteristics, Input data, Performance, Variable contribution, Partial Response Curves, Confidence"),") on the right of the screen. Necessarily these outputs are technical in nature, although accompanying text is provided. A non-technical summary of the modelling methodology is provided in the 'Background' section below. This tool was developed by Cefas within funding from The Crown Estate's ",tags$a(href="https://www.thecrownestate.co.uk/en-gb/what-we-do/on-the-seabed/offshore-wind-evidence-and-change-programme/", "Offshore Wind Evidence and Change Programme."))
##########
h3("Background",style=c("color:#044D94"))
p("Seabed sediments, covering large parts of the continental shelf, support a huge diversity of animal life, collectively known as the ",tags$a(href="https://en.wikipedia.org/wiki/Benthos", "benthos.")," These organisms play an important role in the functioning of marine ecosystems.",
br(),
br(),
"Understanding of the benthos comes from seabed sediment samples collected using ",tags$i("inter alia"),"grab, core and trawl devices. These samplers, deployed from survey vessles, bring back material which is then processed, resulting in a species-by-sample matrix for counts and/or biomass.",
br(),
br(),
"Resulting data are typically used to generate ",tags$a(href="http://www.coastalwiki.org/wiki/Measurements_of_biodiversity", "biodiversity indices,"),
"the simplest of which include number of taxa and abundance. In addition, data can also put through a clustering process to identify groups of samples which support a similar complement of animals, otherwise known as an assemblage.",
br(),
br(),
"Thus we know a lot about the benthos at these point sampled locations. However, even with the 44,603 grab and core samples available through the",tags$a(href="https://sway.office.com/HM5VkWvBoZ86atYP?ref=Link/", "OneBenthic")," big data initiative (Jan 2022), this still only equates to 4,4603 m",tags$sup("2"),", approximately two thirds the area of a football pitch. In order to make informed decisions around marine licensing and conservation, it's vital that there is an understanding of seabed biodiversity across all areas, not just those where we have data. This is where modelling comes in.")
knitr::include_graphics("www/modelling_infographic.png")
br()
br()
p("Here's how it works. We start with a variety of maps for different environmental variables which are considered to be important for the benthos (e.g. depth, temperature, salinity, sediment type). These layers are called",tags$b("predictor variables")," and are available in raster format. Rasters are simply gridded maps in which each grid cell contains a value. For a full list of the raster predictor layers used for modelling see the ",tags$b("Data tab."),
br(),
br(),
"Next the point sample data ", tags$b("(response variable(s)")," are split into 'train' and 'test' subsets. The test subset is held back from modelling and used to assess model performance (i.e how well it represents reality). Train data are overlaid on the raster stack of predictor layers. Looking down through the stack we then extract values for each predictor variable. Thus we have a good understanding of the environmental conditions associated with point sample response variable(s). Note that the point sample data could be counts or classes. We then use this understandning to look back up through the raster stack and predict what the response variable is likely to be in places where we have no point sample data. The modelling approach used in this tool is called",tags$a(href="https://en.wikipedia.org/wiki/Random_forest", "random forest."),
br(),
br(),
"The process is repeated several time, using different subsets of train and test data, resulting in multiple versions of the model. Different model outputs are then comared. Where there is agreement between  model runs then this gives high confidence. Where models differ then we have less confidence. The test set is then overlaid on the final consensus model output to determine performance. For a more in depth description of the methodology see ", tags$a(href="https://doi.org/10.1016/j.envsoft.2018.07.014", "here."),
#knitr::include_graphics("www/output-onlinegiftools.gif")#this works
br(),
br(), 
p("To help interpret model outputs we provide a variety of outputs shown in various tabs (see table below). The important thing to remember is to look at both the model and the confidence."))
br()
############
...
tabexplan <- data.frame(
  ...
  Explanation=c("Description",
         "Modelled class characteristics (categorical data only)",
               ...
               "Relative contribution of predictor variables to the model",
               "Effect of predictor varaibles on response variable",
