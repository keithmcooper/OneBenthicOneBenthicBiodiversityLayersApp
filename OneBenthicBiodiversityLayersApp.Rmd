...
p("This app provides modelled layers for different aspects of benthic biodiversity. Models are based on ",tags$a(href="https://en.wikipedia.org/wiki/Random_forest", "random forest") ," modelling of point sample data from the ",tags$a(href="https://sway.office.com/HM5VkWvBoZ86atYP?ref=Link/", "OneBenthic")," big data initiative. Each model is accompanied by an associated confidence map based on methodology ", tags$a(href="https://doi.org/10.1016/j.envsoft.2018.07.014", "here.")," Understanding of benthic biodiversity is vital for effective decision concerning marine spatial planning, marine licensing, conservation, ",tags$a(href="https://www.marine-ecosystems.org.uk/Research_outcomes/Natural_Capital", "marine natural capital,"),
...
br(),
br(),
"Here's how it works. We start with a variety of maps for different environmental variables which are considered to be important for the benthos (e.g. depth, temperature, salinity, sediment type). These layers are called",tags$b("predictor variables")," and are available in raster format. Rasters are simply gridded maps in which each grid cell contains a value. For a full list of the raster predictor layers used for modelling see the ",tags$b("data tab."), "Next we overlay point sample data", tags$b("(response variable(s)")," on the stack of raster predictor layers. We can then look down through the stack and extract values for each predictor variable. Thus we have a good understanding of the environmental conditions associated with point sample response variable(s). Note that the point sample data could be counts or classes. We then use this understandning to look back up through the raster stack and predict what the response variable is likely to be in places where we have no data. The modelling approach used in this tool is called",tags$a(href="https://en.wikipedia.org/wiki/Random_forest", "random forest."))
br()
#knitr::include_graphics("www/output-onlinegiftools.gif")#this works
knitr::include_graphics("www/modelling_infographic.png")
br()
p("When looking at any model it's important to consider performance (i.e how well it represents reality). To do this we hold back some of the point sample data purely for testing. Are the values from our samples the same or similar to those predicted by the model? Furthermore, we run the model multiple times using random subsets of the data. These multiple model outputs can then be compared. Where there is agreement between  model runs then this gives high confidence. Where models differ then we have less confidence.",
br(),
br(),  
"To help interpret model outputs we provide a variety of outputs shown in tabs. The important thing to remember is to look at both the model and the confidence. " )
############
