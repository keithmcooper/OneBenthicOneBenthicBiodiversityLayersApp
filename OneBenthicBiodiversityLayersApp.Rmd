...
p("Here's how it works. We start with a variety of maps for different environmental variables which are considered to be important for the benthos (e.g. depth, temperature, salinity, sediment type). These layers are called",tags$b("predictor variables")," and are available in raster format. Rasters are simply gridded maps in which each grid cell contains a value. For a full list of the raster predictor layers used for modelling see the ",tags$b("Data tab."),"Next the point sample data ", tags$b("(response variable(s)")," are split into 'train' and 'test' subsets. The test subset is held back from modelling and used to assess model performance (i.e how well it represents reality). Train data are overlaid on the raster stack of predictor layers. Looking down through the stack we then extract values for each predictor variable. Thus we have a good understanding of the environmental conditions associated with point sample response variable(s). Note that the point sample data could be counts or classes. We then use this understandning to look back up through the raster stack and predict what the response variable is likely to be in places where we have no point sample data. The modelling approach used in this tool is called",tags$a(href="https://en.wikipedia.org/wiki/Random_forest", "random forest.")," The process is repeated several time, using different subsets of train and test data, resulting in multiple versions of the model. Different model outputs are then comared. Where there is agreement between  model runs then this gives high confidence. Where models differ then we have less confidence. The test set is then overlaid on the final consensus model output to determine performance. For a more in depth description of the methodology see ", tags$a(href="https://doi.org/10.1016/j.envsoft.2018.07.014", "here."))
