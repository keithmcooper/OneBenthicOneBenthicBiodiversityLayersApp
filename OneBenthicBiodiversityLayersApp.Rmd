...
h2("Modelling Methodology",style=c("color:#044D94"))
p("These layers were created using data from the",tags$a(href="https://sway.office.com/HM5VkWvBoZ86atYP?ref=Link/", "OneBenthic")," big data initiative. Models are based on Random forest modelling of point sample data using a variety of raster predictor layers obtained from", tags$a(href="https://www.bio-oracle.org/", "Bio-ORACLE"), "and",tags$a(href="https://doi.org/10.1016/j.envsoft.2018.07.014", "Mitchell et al. (2019)."))
br()

p("Mitchell, P.J., Downie, A.-L., Diesing, M. How good is my map? 2018. A tool for semi-automated thematic mapping and spatially explicit confidence assessment. Env. Model. Softw. 108, 111–122.", tags$a(href="https://doi.org/10.1016/j.envsoft.2018.07.014", "https://doi.org/10.1016/j.envsoft.2018.07.014"))
br()

## Header: How to use this app
h2("How to use this app",style=c("color:#044D94"))
p("Using the selection boxes on the left of the screen, select a category of modelled layer, choosing from the following options: Biodiversity, Structure, Traits or Species distibution model (SDM). Next select a layer to display.","The map window contains two tabs, one for the model itself and the other for an associated confidence map. The map also allows for different polygons to be displayed, including European offshore windfarm areas (based on an api link to Emodnet).")
br()
p("On the right of the screen are a number of tabs which show various model outputs. The first tab ‘About’ provides a description of the layers, a citation (if appropriate) and details of funders or partners involved in the production of the layer. The second tab ‘Characteristics’ applies only to categorical layers and details the nature or characteristics of individual groups. The 3rd tab ‘Input’ shows a histogram of the data used in the model. The 4th tab ‘Model performance’ is based on test data (i.e. data not included in the model development), with actual values of the modelled parameter plotted against modelled values. In a perfect model all values would lie on a diagonal line. The R2 value provides a numeric value describing the relationship between observed versus predicted values, with 1 being perfect agreement and 0 being no agreement. Typically a value of ? is considered to be good. The 5th tab ‘Variable contribution’ shows how the modelled metric changes in response to increasing values of each predictor variable.")
br()
## Header: Funders
...
#,"font-size: 30px"
 output$desc <-  renderUI({
    ...
    if(as.character(input$layernameInput) == "Assemblages"){
      list(h2("Description",style=c("color:#044D94")),"This layer shows benthic  macrofaunal assemblages (biotopes) based on the methodology in Cooper et al. (2019), but using an expanded dataset (see Cooper, Downie and Curtis (2021) for further details. In summary, macrofaunal abundance data from comparable surveys (0.1m2 grab or core, processed over a 1mm sieve, and not taken from known impact sites) were fourth-root transformed prior to k-means clustering. Cluster group distribution was then modelled using a random forest approach.")}
    #else if(as.character(input$layernameInput) == "Richness"){
   ...
   # else if(as.character(input$layernameInput) == "Response"){
    ...
    #  list("seven")}
      ...
      list(h2("Description",style=c("color:#044D94")),br(),"Add text")}
   else if(as.character(input$layernameInput) == "Modiolus_modiolus"){
      list(h2("Description",style=c("color:#044D94")),"This model consist of the predicted abundance of",em("Modiolus modiolus"), "over the study area as mean square-root transformed counts and the coefficient of variation (CV), both calculated over the ten split sample cross-validation runs (see 'Confidence' tab). The model predicts patches of high abundance mainly in the Irish Sea and the Northern North Sea. High abundance predicted in the deep waters on the shelf break and in the Norwegian Trench are expected to be model artefacts as there is no data supporting the predicted distribution in those areas (see 'Input data' tab). The coefficient of variation (CV) of predictions from the 10 cross-validation runs stays mostly below 25% with patches of highly variable predictions (up to 60% of the mean prediction).")}
    else if(as.character(input$layernameInput) == "Ampelisca_spinipes"){
      list(h2("Description",style=c("color:#044D94")),br(),"The model predicts patches of high abundance around the Southern North Sea, English Channel and Irish Sea up the coast of Western Scotland. The coefficient of variation (CV) of predictions from the 10 cross-validation runs stays mostly below 10% and almost entirely below 25%.")}
       else if(as.character(input$layernameInput) == "Arctica_islandica"){
      list(h2("Description",style=c("color:#044D94")),br(),"Add text")}
       else if(as.character(input$layernameInput) == "Crepidula_fornicata"){
      list(h2("Description",style=c("color:#044D94")),br(),"Add text")}
...
if(as.character(input$layernameInput) == "Assemblages"){list(h2("Records",style=c("color:#044D94")),p("Some text 1"))}
else if(as.character(input$layernameInput) == "Sabellaria_spinulosa"){list(h2("Records",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Modiolus_modiolus"){list(h2("Records",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Ampelisca_spinipes"){list(h2("Records",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Arctica_islandica"){list(h2("Records",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Crepidula_fornicata"){list(h2("Records",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Goniadella_gracilis"){list(h2("Records",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Abra_alba"){list(h2("Records",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Lagis_koreni"){list(h2("Records",style=c("color:#044D94")),p("Some text 1"))}
  })
...
if(as.character(input$layernameInput) == "Assemblages"){list(h2("Plot",style=c("color:#044D94")),p("Some text 1"))}
else if(as.character(input$layernameInput) == "Sabellaria_spinulosa"){list(h2("Plot",style=c("color:#044D94")),p("Some text 1"))}  
  else if(as.character(input$layernameInput) == "Modiolus_modiolus"){list(h2("Plot",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Ampelisca_spinipes"){list(h2("Plot",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Arctica_islandica"){list(h2("Plot",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Crepidula_fornicata"){list(h2("Plot",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Goniadella_gracilis"){list(h2("Plot",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Abra_alba"){list(h2("Plot",style=c("color:#044D94")),p("Some text 1"))}
  else if(as.character(input$layernameInput) == "Lagis_koreni"){list(h2("Plot",style=c("color:#044D94")),p("Some text 1"))}
  })
uiOutput("plotheader")
...
output$valstatsheader <-  renderUI({
    if(as.character(input$layernameInput) == "Assemblages"){list(h2("Validation statistics",style=c("color:#044D94")),p("Some text 1"))}
else if (as.character(input$layernameInput) == "Sabellaria_spinulosa"){list(h2("Validation statistics",style=c("color:#044D94")),p("Some text 1"))}
else if (as.character(input$layernameInput) == "Modiolus_modiolus"){list(h2("Validation statistics",style=c("color:#044D94")),p("Some text 1"))}
else if (as.character(input$layernameInput) == "Ampelisca_spinipes"){list(h2("Validation statistics",style=c("color:#044D94")),p("Some text 1"))}
else if (as.character(input$layernameInput) == "Arctica_islandica"){list(h2("Validation statistics",style=c("color:#044D94")),p("Some text 1"))}
else if (as.character(input$layernameInput) == "Crepidula_fornicata"){list(h2("Validation statistics",style=c("color:#044D94")),p("Some text 1"))}
else if (as.character(input$layernameInput) == "Goniadella_gracilis"){list(h2("Validation statistics",style=c("color:#044D94")),p("Some text 1"))}
else if (as.character(input$layernameInput) == "Abra_alba"){list(h2("Validation statistics",style=c("color:#044D94")),p("Some text 1"))}
else if (as.character(input$layernameInput) == "Lagis_koreni"){list(h2("Validation statistics",style=c("color:#044D94")),p("Some text 1"))}
...
if(as.character(input$layernameInput) == "Sabellaria_spinulosa"){list(h2("Observed vs predicted values",style=c("color:#044D94")),p("Some text 1"))}
else if(as.character(input$layernameInput) == "Arctica_islandica"){list(h2("Observed vs predicted values",style=c("color:#044D94")),p("Some text 1"))}
else if(as.character(input$layernameInput) == "Modiolus_modiolus"){list(h2("Observed vs predicted values",style=c("color:#044D94")),p("Some text 1"))}
else if(as.character(input$layernameInput) == "Ampelisca_spinipes"){list(h2("Observed vs predicted values",style=c("color:#044D94")),p("Some text 1"))} 
else if(as.character(input$layernameInput) == "Arctica_islandica"){list(h2("Observed vs predicted values",style=c("color:#044D94")),p("Some text 1"))}
else if(as.character(input$layernameInput) == "Crepidula_fornicata"){list(h2("Observed vs predicted values",style=c("color:#044D94")),p("Some text 1"))}
else if(as.character(input$layernameInput) == "Goniadella_gracilis"){list(h2("Observed vs predicted values",style=c("color:#044D94")),p("Some text 1"))} 
else if(as.character(input$layernameInput) == "Abra_alba"){list(h2("Observed vs predicted values",style=c("color:#044D94")),p("Some text 1"))}
else if(as.character(input$layernameInput) == "Lagis_koreni"){list(h2("Observed vs predicted values",style=c("color:#044D94")),p("Some text 1"))}
else if(as.character(input$layernameInput) == "Assemblages"){list(h2("Confusion Matrix",style=c("color:#044D94")),p("Some text 1"))}
})
